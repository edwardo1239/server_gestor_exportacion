...
class InventariosRepository {
    ...
    static async post_inventarios_canastillas_registro(req) {
        try {
            ...
            // se restan las canastillas tanto en los predios como en el inventario
            // de la planta

            if (accion === "ingreso") {

                const proveedorOrigen = await ProveedoresRepository.get_proveedores({
                    query: { PREDIO: origen },
                    select: { canastillas: 1 }
                })


                if (proveedorOrigen.length > 0) {
                    const newCanastillas = proveedorOrigen[0].canastillas - (canastillas + canastillasPrestadas)
                    await ProveedoresRepository.modificar_proveedores(
                        { _id: proveedorOrigen[0]._id },
                        {
                ...
                const proveedorDestino = await ProveedoresRepository.get_proveedores({
                    query: { PREDIO: destino },
                    select: { canastillas: 1 }
                })

                if (proveedorDestino.length > 0) {
                    const newCanastillas = proveedorDestino[0].canastillas + (canastillas + canastillasPrestadas)
                    await ProveedoresRepository.modificar_proveedores(
                        { _id: proveedorDestino[0]._id },
                        {
                            $set: { canastillas: newCanastillas }
                        }
                    )
